...
var getHoverCardFromTitle = function(title, lang) {
	var apiDomain = lang;
	var url = 'https://' + apiDomain + '.wikipedia.org/w/api.php';
	return $.ajax({
		dataType: 'json',
		type: 'GET',
		url: url,
		...
		success: function(response) {
			...
			if (response.error != null) {
				//analytics.gaEvent('hovercard', 'hovercard-api-response-error', response.error);
				console.log("OCD: got error", response);
			} else {
				console.log('response from wikipedia ', response);
				//analytics.gaEvent('hovercard', 'hovercard-api-response-ok');
				...
				href = 'https://www.wikiwand.com/' + apiDomain + '/' + page.title.split(' ').join('_');
				html = '';
				...
				if (disambig) {
					...
					page.links.forEach(function(link, index) {
						console.log(link.title, ':', page.title);
						if (count < 12 && link.title.toLowerCase().indexOf(page.title.toLowerCase()) > -1) {
							console.log('match');
							count++;
							disambigHtml += '<a class="ww_disambig_option" target="_blank" href="http://www.wikiwand.com/' + lang +
				...
				$('.ww_feedback_button').on('click', function(event) {
					...
					window.open("http://mail.google.com/mail/?view=cm&fs=1&tf=1&to=popup@wikiwand.com&su=Wikiwand+quick-definition+popup+feedback");
				});
